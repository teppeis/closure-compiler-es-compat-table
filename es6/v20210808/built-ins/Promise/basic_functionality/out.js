...
$jscomp.polyfillIsolated = function(c, e, f, h) {
  ...
  e = e(f);
  null != e && (c ? $jscomp.defineProperty($jscomp.polyfills, d, {configurable:!0, writable:!0, value:e}) : e !== f && (void 0 === $jscomp.propertyToPolyfillSymbol[d] && (f = 1e9 * Math.random() >>> 0, $jscomp.propertyToPolyfillSymbol[d] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(d) : $jscomp.POLYFILL_PREFIX + f + "$" + d), $jscomp.defineProperty(h, $jscomp.propertyToPolyfillSymbol[d], {configurable:!0, writable:!0, value:e})));
};
$jscomp.polyfill("Promise", function(c) {
  ...
  d.prototype.then = function(a, b) {
    ...
    this.callWhenSettled_(g(a, k), g(b, m));
    return t;
  };
  ...
  d.race = function(a) {
    return new d(function(b, g) {
      for (var k = $jscomp.makeIterator(a), m = k.next(); !m.done; m = k.next()) {
        f(m.value).callWhenSettled_(b, g);
      }
    });
  };
  d.all = function(a) {
    var b = $jscomp.makeIterator(a), g = b.next();
    return g.done ? f([]) : new d(function(k, m) {
      function t(q) {
        return function(r) {
          n[q] = r;
          p--;
      ...
      var n = [], p = 0;
      do {
        n.push(void 0), p++, f(g.value).callWhenSettled_(t(n.length - 1), m), g = b.next();
      } while (!g.done);
    });
  };
  return d;
